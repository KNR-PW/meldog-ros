...
int main(int argc, char** argv)
{
    ...
    // set stop to moteus
    moteus_wrapper_ptr->init_to_tx_frame(tx_frame);
    ...
    {
        moteus_wrapper_ptr->start_pos_to_tx_frame(tx_frame, controller_command);
        pi3hat_output = pi3hat.Cycle(input);
        ::usleep(2000);
        moteus_wrapper_ptr->rx_frame_to_state(rx_frame, controller_state);
    }
    ...
    {   
        ...
        moteus_wrapper_ptr->command_to_tx_frame(tx_frame, controller_command);
        pi3hat_output = pi3hat.Cycle(input);
        ::usleep(1000);
